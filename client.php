            ...
            <?php foreach ($profiles as $profile): ?>
                <div class="profile-box">
                    ...
                    <?php if (isset($profile['profile_picture']) && !empty($profile['profile_picture'])): ?>
                        <a href="profile.php?user_id=<?= $profile['user_id'] ?>"><img src="<?= htmlspecialchars($profile['profile_picture'], ENT_QUOTES, 'UTF-8') ?>" alt="Profile Picture" class="profile-picture"></a>
                    <?php else: ?>
                        <a href="profile.php?user_id=<?= $profile['user_id'] ?>"><div class="profile-picture initials"><?= $initials ?></div></a>
                    <?php endif; ?>
                    
                    <!-- Linking to the lawyer's profile & showing profile info -->
                    <h2><a href="profile.php?user_id=<?= $profile['user_id'] ?>"><?= htmlspecialchars($profile['full_name'], ENT_QUOTES, 'UTF-8') ?></h2>
